...
static unsigned short crc16(unsigned short crc, const void *data, size_t size)
{
	unsigned char x;
	const uint8_t *p;
...
int crc_push(struct crc *crc, int crctype)
{
	crc->n++;
	if(crc->n > 7) return -1;
	crc->crctype[crc->n] = crctype;
	crc->crc16[crc->n] = 0xffff;
	crc->crc32[crc->n] = 0x0;
	return 0;
}
...
void crc_update(struct crc *crc, const void *data, size_t size)
{
	...
	if(crc->n == -1) return;
	for(i=0;i<=crc->n;i++) {
		if(crc->crctype[i] == CRC32)
			crc->crc32[i] = crc32(crc->crc32[i], data, size);
		if(crc->crctype[i] == CRC16)
			crc->crc16[i] = crc16(crc->crc16[i], data, size);
	}
}
